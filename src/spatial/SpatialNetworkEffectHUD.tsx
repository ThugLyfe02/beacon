...
interface Props {
  state: SpatialNetworkEffectState;
  accent: string;
  onPrimaryAction: () => void;
}

export default function SpatialNetworkEffectHUD({ state, accent, onPrimaryAction }: Readonly<Props>) {
    ...
    <View pointerEvents="box-none" style={styles.wrap}>
      <View style={[styles.card, { borderColor: `${accent}55` }]}>
        <View style={styles.headerRow}>
          <Text style={[styles.eyebrow, { color: accent }]}>COMPOUNDING LOOP</Text>
          <Text style={styles.score}>{Math.round(state.compoundingScore * 100)}% leverage</Text>
        </View>
        ...
        <View style={styles.footerRow}>
          <View style={styles.metricBlock}>
            <Text style={styles.metricValue}>{Math.round(primary.confidence * 100)}%</Text>
            <Text style={styles.metricLabel}>confidence</Text>
          </View>
          <View style={styles.metricBlock}>
            <Text style={styles.metricValue}>{Math.round(primary.leverage * 100)}%</Text>
          ...
          <Pressable accessibilityRole="button" onPress={onPrimaryAction} style={[styles.button, { borderColor: accent }]}>
            <Text style={[styles.buttonText, { color: accent }]}>Open loop</Text>
          </Pressable>
        </View>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  score: { color: '#64748B', fontSize: 9, fontWeight: '800' },
  ...
  metricBlock: { minWidth: 54 },
  metricValue: { color: '#E2E8F0', fontSize: 12, fontWeight: '900' },
  metricLabel: { marginTop: 1, color: '#64748B', fontSize: 8 },
  button: {
    marginLeft: 'auto',
    minHeight: 32,
