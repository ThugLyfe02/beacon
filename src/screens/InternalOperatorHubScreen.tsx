...
const LABS: LabDefinition[] = [
  ...
  { route: 'InternalAssumptionStaleness', title: 'Assumption Staleness', code: 'REVALIDATE', description: 'Revalidates open hypotheses when canonical graph version, evidence quality, incident pressure, chronology, route diversity, conflict dependencies, or bottleneck structure materially changes. Stale means review due—not false.', capability: 'graph_manage', accent: 'accent' },
  { route: 'InternalReasoningLineage', title: 'Reasoning Lineage', code: 'PROVENANCE', description: 'Exact hypothesis→evidence dependency graph, conflict blast radius, orphaned refs after graph evolution, and explicit operator rebinding for legacy hypotheses. Dependencies are recorded, never inferred.', capability: 'graph_manage', accent: 'accent' },
  { route: 'InternalDecisionReproducibility', title: 'Decision Reproducibility', code: 'RECEIPTS', description: 'Server-sealed analytical receipts over canonical graph version + exact evidence-ref digest + bounded policy context, with later drift/conflict/orphan comparison. Reproducibility never certifies truth.', capability: 'graph_manage', accent: 'accent' },
  { route: 'InternalReasoningRevalidation', title: 'Revalidation Queue', code: 'REVIEW DEBT', description: 'Durable artifact-level remediation tasks created from exact reasoning dependencies. Acknowledge and resolve review obligations without changing graph evidence or hypothesis outcomes.', capability: 'graph_manage', accent: 'accent' },
  { route: 'InternalCollaborativeCaseMemory', title: 'Collaborative Case Memory', code: 'SHARED MEMORY', description: 'Durable Casebook chronology plus per-operator rehydration: assignment, new evidence memory, unresolved questions, canonical graph change and recommended next analysis.', capability: 'graph_manage', accent: 'accent' },
  ...
  { route: 'InternalEvidenceDebt', title: 'Evidence Debt', code: 'VERIFY', description: 'Self-healing verification queue that ranks ambiguous, stale, one-shot, weak-context, route-bottleneck, and operator-confirmed evidence conflicts by expected analytical-authority gain.', capability: 'graph_read', accent: 'accent' },
  { route: 'InternalEvidenceConflicts', title: 'Evidence Conflict Ledger', code: 'CONFLICT', description: 'Operator-confirmed, graph-version-bound evidence disagreements plus non-authoritative tension suggestions. Both source edges are preserved until human review resolves or dismisses the conflict.', capability: 'graph_manage', accent: 'accent' },
        ...
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            <View style={{ flex: 1 }}>
              ...
              <NeonText variant="bodyMuted">Adaptive command · falsifiable decision memory · exact reasoning lineage · reproducibility receipts · durable revalidation queue · calibration + retrospectives · assumption revalidation · evidence conflicts · collaborative Casebook rehydration · temporal coherence · Perspectives · evidence debt · ontology drift · Watchtower · routing · forensic pivots · Machines · longitudinal memory</NeonText>
            </View>
          ...
          <Surface padded style={styles.autonomyCard}>
            ...
            <NeonText variant="bodyMuted" style={{ marginTop: spacing.sm, lineHeight: 19 }}>Constellation may detect, diff, route, recommend analytical Perspectives and structural pattern queries, rank graph-level evidence debt, observe ontology/schema drift, test temporal coherence, maintain bounded collaborative Casebook memory, rehydrate cases, preserve exact decision→evidence lineage, seal bounded analytical reproducibility receipts, create bounded revalidation obligations from exact dependencies, flag stale assumptions for review, suggest non-authoritative evidence tensions, preserve operator-confirmed conflict state, monitor structural conditions, reconcile mission memory, replay private recipes, calibrate its analytical method, and learn shrinkage-adjusted retrospective associations between decision-time conditions and later hypothesis outcomes. Current evidence always sets the authority ceiling; calibration, staleness, conflict state, and retrospective history may only increase scrutiny or reduce authority. Reproducibility is not truth certification, no lineage edge is inferred, revalidation never changes truth automatically, no conflict resolution rewrites source evidence, and none of these systems may message attendees, manufacture relationships, bypass blocks, or execute social interventions without explicit human approval.</NeonText>
          </Surface>
        </ScrollView>
      </SafeAreaView>
    </View>
