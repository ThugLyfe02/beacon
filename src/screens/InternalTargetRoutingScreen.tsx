...
import { analyzeInternalRouteTemporalCoherence } from '../admin/InternalAgenticTimelineEngine';
        ...
        <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
          <View style={styles.header}>
            <View style={{ flex: 1 }}>
              ...
              <NeonText variant="bodyMuted">Diverse paths · provenance quality · temporal coherence · bottleneck exposure · safety suppression · observational priors</NeonText>
            </View>
          ...
          <Surface padded style={styles.contractCard}>
            ...
            <NeonText variant="bodyMuted" style={styles.bodyGap}>A route means the authorized graph contains an explainable structural path. It does not estimate compatibility, consent, influence, or whether an introduction should occur. Temporal coherence describes overlap of retained observation windows; no overlap means historical-only caution, not that an edge was false. Person-person hops are removed when the authoritative block-suppression set forbids them.</NeonText>
          </Surface>
          ...
          {portfolio ? <>
            ...
            <Section title="ROUTE PORTFOLIO" subtitle="Routes are diversity-aware and temporally annotated: structural reach across history is kept distinct from contemporaneous support.">
              {portfolio.routes.map((route, index) => {
                const temporal = analyzeInternalRouteTemporalCoherence(route);
                return <Surface key={route.id} elevated padded style={[styles.routeCard, !temporal.hasSharedObservationWindow ? styles.temporalWarning : null]}>
                  ...
                  <View style={styles.routeMetrics}><Mini label="EVIDENCE" value={`${Math.round(route.evidenceQuality * 100)}%`} /><Mini label="FRESHNESS" value={`${Math.round(route.freshness * 100)}%`} /><Mini label="TEMPORAL" value={`${Math.round(temporal.overlapScore * 100)}%`} /><Mini label="REDUNDANCY" value={`${Math.round(route.redundancy * 100)}%`} /></View>

                  {!temporal.hasSharedObservationWindow ? <Surface padded style={styles.temporalCard}><Pill label="HISTORICAL-ONLY CAUTION" tone="accent" dot /><NeonText variant="bodyMuted" style={{ marginTop: spacing.sm }}>No single retained observation window contains every edge in this route. Nearest aggregate gap ~{Math.round(temporal.nearestGapDays)} days. Review Timeline before treating this path as contemporaneously supported.</NeonText></Surface> : null}
