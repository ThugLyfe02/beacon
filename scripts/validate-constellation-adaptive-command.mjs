...
for (const [text, why] of [
  ...
  ['openIncidents.slice(0, 8)', 'Command must surface bounded open incident workload'],
  ["incident.severity === 'critical' || incident.severity === 'high'", 'Command must visually distinguish high-urgency correlated conditions'],
  ['No open correlated Watchtower incidents.', 'Command must explicitly disclose when correlated incident workload is clear'],
